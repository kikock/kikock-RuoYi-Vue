...
public class WfModelServiceImpl extends FlowServiceFactory implements IWfModelService{
    ...
    @Override
    public List<WfModelVo> listPage(WfModelBo modelBo){
        List<WfModelVo> result = new ArrayList<>();
        //分页数据
    ...
    @Override
    public List<WfModelVo> historyList(WfModelBo modelBo){
        List<WfModelVo> result = new ArrayList<>();
        //分页数据
        PageDomain pageDomain = TableSupport.buildPageRequest();
        Integer pageNum = pageDomain.getPageNum();
    ...
    public void saveModel(WfModelBo modelBo){
        ...
        if (StrUtil.isBlank(startEvent.getFormKey())) {
            throw new RuntimeException("请配置开始节点流程表单!!");
        }
        Model newModel;
