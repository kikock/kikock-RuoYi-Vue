...
public class WfProcessServiceImpl extends FlowServiceFactory implements IWfProcessService{
    ...
    public List<WfDefinitionVo> selectPageStartProcessList(ProcessQuery processQuery){
        List<WfDefinitionVo> result = new ArrayList<>();
        //分页数据
    ...
    @Override
    public List<WfTaskVo> selectPageOwnProcessList(ProcessQuery processQuery){
        List<WfTaskVo> result = new ArrayList<>();
        //分页数据
        PageDomain pageDomain = TableSupport.buildPageRequest();
        Integer pageNum = pageDomain.getPageNum();
        Integer pageSize = pageDomain.getPageSize();
...
//        page.setTotal(historicProcessInstanceQuery.count());
    ...
    @Override
    public List<WfTaskVo> selectPageTodoProcessList(ProcessQuery processQuery){
        List<WfTaskVo> result = new ArrayList<>();
        //分页数据
        PageDomain pageDomain = TableSupport.buildPageRequest();
        Integer pageNum = pageDomain.getPageNum();
...
//        page.setTotal(taskQuery.count());
        ...
        for (Task task : taskList) {
            ...
            flowTask.setStartUserName(sysUserSimpleVo.getName());
    ...
    public List<WfTaskVo> selectTodoProcessList(ProcessQuery processQuery){
        ...
        for (Task task : taskList) {
            ...
            taskVo.setStartUserName(sysUserSimpleVo.getName());
    ...
    @Override
    public List<WfTaskVo> selectPageClaimProcessList(ProcessQuery processQuery){
        List<WfTaskVo> result = new ArrayList<>();
        //分页数据
        PageDomain pageDomain = TableSupport.buildPageRequest();
        Integer pageNum = pageDomain.getPageNum();
    ...
    @Override
    public List<WfTaskVo> selectPageFinishedProcessList(ProcessQuery processQuery){
        List<WfTaskVo> result = new ArrayList<>();
        //分页数据
        PageDomain pageDomain = TableSupport.buildPageRequest();
        Integer pageNum = pageDomain.getPageNum();
    ...
    public FormConf selectFormContent(String definitionId, String deployId, String procInsId){
        ...
        WfDeployForm wfDeployForm = new WfDeployForm(deployId, startEvent.getFormKey(), startEvent.getId());

        WfDeployFormVo wfDeployFormVo = deployFormMapper.selectVoOneWfDeployForm(wfDeployForm);
        if (ObjectUtil.isEmpty(wfDeployFormVo)) {
            throw new RuntimeException("获取流程表单失败！");
        }
        FormConf formConf = JSONUtil.toBean(wfDeployFormVo.getContent(), FormConf.class);
    ...
    @Override
    public WfDeployFormVo selectDeployFormContent(String definitionId, String deployId, String procInsId){
        BpmnModel bpmnModel = repositoryService.getBpmnModel(definitionId);
        if (ObjectUtil.isNull(bpmnModel)) {
            throw new RuntimeException("获取流程设计失败！");
        }
...
//            detailVo.setTaskFormData(currTaskFormData(historicProcIns.getDeploymentId(), taskIns));
        ...
        List<WfDeployFormVo> wfDeployFormVos = processFormData(bpmnModel, historicProcIns);
        WfViewerVo flowViewer = getFlowViewer(bpmnModel, procInsId);
        detailVo.setFlowViewer(flowViewer);
        detailVo.setProcessFormData(wfDeployFormVos.stream().filter(wfDeployFormVo -> wfDeployFormVo.isStart() == true)
     ...
     * 获取当前任务流程表单信息
     */
    private WfDeployFormVo currNewTaskFormData(String deployId, HistoricTaskInstance taskIns){
        Map<String,Object> taskLocalVariables = taskIns.getProcessVariables();
        WfDeployForm wfDeployForm = new WfDeployForm(deployId, taskIns.getFormKey(), taskIns.getTaskDefinitionKey());
        WfDeployFormVo deployFormVo = deployFormMapper.selectVoOneWfDeployForm(wfDeployForm);
        if (ObjectUtil.isNotEmpty(deployFormVo)) {
            deployFormVo.setVariables(taskLocalVariables);
            return deployFormVo;
        }
        return null;
    }
...
//                .eq(localScope, WfDeployForm::getNodeKey, flowElement.getId()));
            ...
            List<WfDeployForm> wfDeployForms = deployFormMapper.selectWfDeployFormList(wfDeployForm);

            //@update by Brath：避免空集合导致的NULL空指针
            WfDeployForm formInfo = wfDeployForms.stream().findFirst().orElse(null);
            ...
            if (ObjectUtil.isNotNull(formInfo)) {
                ...
                FormConf formConf = JSONUtil.toBean(formInfo.getContent(), FormConf.class);
                if (null != formConf) {
    ...
    private List<WfDeployFormVo> processFormData(BpmnModel bpmnModel, HistoricProcessInstance historicProcIns){
        ...
        for (HistoricActivityInstance activityInstance : activityInstanceList) {
            ...
            if (flowElement instanceof StartEvent) {
                //标记开始节点表单
                deployFormVo.setStart(true);
            }
            deployFormVo.setVariables(variables);

            formInfo.add(deployFormVo);
        }
        return formInfo;
    }
    ...
    private void buildStartFormData(HistoricProcessInstance historicProcIns, Process process, String deployId, List<FormConf> procFormList){
        ...
        WfDeployFormVo startFormInfo = deployFormMapper.selectVoOneWfDeployForm(wfDeployForm);
    ...
    private void buildUserTaskFormData(String procInsId, String deployId, Process process, List<FormConf> procFormList){
        ...
        for (HistoricActivityInstance instanceItem : activityInstanceList) {
            ...
            WfDeployForm wfDeployForm = new WfDeployForm(deployId, formKey, userTask.getId());
        ...
        for (HistoricActivityInstance activityInstance : historicActivityInstanceList) {
            ...
            if (BpmnXMLConstants.ELEMENT_EVENT_START.equals(activityInstance.getActivityType())) {
                if (ObjectUtil.isNotNull(historicProcIns)) {
                    ...
                    SysUserSimpleVo sysUserSimpleVo = userService.findSimpleUserById(userId);
                    if (ObjectUtil.isNotNull(sysUserSimpleVo)) {
