...
public class WfTaskServiceImpl extends FlowServiceFactory implements IWfTaskService{
    ...
    public void complete(WfTaskBo taskBo) {
        ...
        // 处理抄送用户
        if (!copyService.makeCopy(taskBo).isSuccess()) {
    ...
    public void taskReturn(WfTaskBo bo) {
        ...
        // 处理抄送用户
        if (!copyService.makeCopy(bo).isSuccess()) {
            throw new RuntimeException("抄送任务失败");
        }
    }
    ...
    public void delegateTask(WfTaskBo bo) {
        ...
        StringBuilder commentBuilder = new StringBuilder(SecurityUtils.getNickName())
            .append("->");
        SysUserSimpleVo simpleUserById = sysUserService.findSimpleUserById(Long.parseLong(bo.getUserId()));


        if (ObjectUtil.isNotNull(simpleUserById)) {
            commentBuilder.append(simpleUserById.getName());
        } else {
    ...
    public void revokeProcess(WfTaskBo taskBo) {
        ...
        for (Task task : activateTaskList) {
            ...
            if (CollUtil.contains(nextUserTaskKeys, task.getTaskDefinitionKey())) {
                ...
                taskService.addComment(task.getId(), task.getProcessInstanceId(), FlowComment.REVOKE.getType(), SecurityUtils.getNickName() + "撤回流程审批");
