    ...
    <div class="my-process-designer__header" style="z-index: 999; display: table-row-group">
      ...
      <template v-if="!$slots['control-header']">
        <ElButtonGroup key="file-control">
          <XButton pre-icon="file-open" title="打开文件" @click="refFile.click()" />
          <ElTooltip effect="light" placement="bottom">
            <template #content>
              <div>
                <XButton style="color: #409eff" pre-icon="file" title="下载为XML文件"  class="mt5" @click="downloadProcessAsXml()" />
                <br />
                <XButton  style="color: #409eff"  pre-icon="file"  title="下载为SVG文件" class="mt5" @click="downloadProcessAsSvg()" />
                <br />
                <XButton   style="color: #409eff" pre-icon="file"  title="下载为BPMN文件" @click="downloadProcessAsBpmn()" />
              </div>
            </template>
            <XButton title="下载文件" pre-icon="file-download" />
          </ElTooltip>
          <el-tooltip effect="light">
            <XButton pre-icon="eye-open" title="浏览" />
            <template #content>
              <XButton style="color: #409eff" pre-icon="1634"  title="预览XML" @click="previewProcessXML" />
              <br />
              <XButton style="color: #409eff" pre-icon="1634"  title="预览JSON" @click="previewProcessJson" />
            </template>
          </el-tooltip>
          <el-tooltip
          ...
          >
            <XButton pre-icon="cpu"  title="模拟" @click="processSimulation" />
          </el-tooltip>
        </ElButtonGroup>
...
<!--        </ElButtonGroup>-->
        <ElButtonGroup key="scale-control">
          ...
          <el-tooltip effect="light" content="放大视图">
            <XButton pre-icon="zoom-out" @click="processZoomIn()" :disabled="defaultZoom > 4" />
          </el-tooltip>
          <el-tooltip effect="light" content="重置视图并居中">
            <!-- <el-button icon="el-icon-c-scale-to-original" @click="processReZoom()" /> -->
            <XButton pre-icon="c-scale-to-original" @click="processReZoom()" />
          </el-tooltip>
        </ElButtonGroup>
        <ElButtonGroup key="stack-control">
          <el-tooltip effect="light" content="撤销">
            <!-- <el-button :disabled="!revocable" icon="el-icon-refresh-left" @click="processUndo()" /> -->
            <XButton pre-icon="forward" @click="processUndo()" :disabled="!revocable" />
          </el-tooltip>
          <el-tooltip effect="light" content="恢复">
            <XButton pre-icon="back" @click="processRedo()" :disabled="!recoverable" />
          </el-tooltip>
          <el-tooltip effect="light" content="重新绘制">
            <XButton pre-icon="refresh" @click="processRestart()" />
          </el-tooltip>
        </ElButtonGroup>
        <XButton
            pre-icon="add"
...
<script  setup name="MyProcessDesigner">
...
const additionalModules = computed(() => {
  console.log("进入additionalModules方法");
  const Modules = []
  // 仅保留用户自定义扩展模块
...
const initBpmnModeler = () => {
  console.log("进入initBpmnModeler方法");
...
const createNewDiagram =  (xml) => {
  console.log("进入方法createNewDiagram =>创建新的流程图");
